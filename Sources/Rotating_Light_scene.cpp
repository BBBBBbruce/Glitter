...
int main()
{
    ...
    Model ourModel(std::string("./Glitter/resource/Models/dragon/dragon.obj").c_str());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = glm::scale(model, glm::vec3(0.35f));	// it's a bit too big for our scene, so scale it down
        //model = glm::rotate(model, 0.785398f, glm::vec3(0.f, 0.2f, 0.5f));

        ourShader.setMat4("model", model);

        float rotationfactor = (glm::sin(currentFrame)-1)/2;
        glm::vec3 lightpos = glm::vec3(5.f, 5.f, 10.f);
        lightpos = glm::rotate(lightpos,  rotationfactor, glm::vec3(0.f, 1.f, 0.f));
